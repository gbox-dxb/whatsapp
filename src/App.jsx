          ...
          delay: 0.2
            ...
            <Card className="p-6 bg-white/10 backdrop-blur-lg border-white/20">
              <div className="space-y-4">
                ...
                <Textarea placeholder="Paste your WhatsApp order messages here...
...
Special Note :   Handle with care" value={inputText} onChange={e => setInputText(e.target.value)} className="min-h-[200px] bg-white/5 border-white/20 text-white placeholder-gray-400 resize-none" />
                <div className="flex gap-3">
                  <Button onClick={handleParse} className="bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white font-semibold px-6 py-2">
                    <MessageSquare className="w-4 h-4 mr-2" />
                    Parse Messages
                  </Button>
                  <Button onClick={() => setInputText('')} variant="outline" className="border-white/20 text-white hover:bg-white/10">
                    Clear Input
                  </Button>
                </div>
              </div>
          ...
          delay: 0.4
            ...
            <Button onClick={handleExportExcel} className="bg-gradient-to-r from-emerald-500 to-teal-600 hover:from-emerald-600 hover:to-teal-700 text-white font-semibold">
              <Download className="w-4 h-4 mr-2" />
              Export to Excel
            </Button>
            <Button onClick={toggleView} variant="outline" className="border-white/20 text-white hover:bg-white/10">
              <Eye className="w-4 h-4 mr-2" />
              {showAllOrders ? 'Show Current Session' : 'Show All Stored Orders'}
            </Button>
            <Button onClick={handleClearDatabase} variant="outline" className="border-red-400/50 text-red-400 hover:bg-red-500/10">
          ...
          {/* Stats */}
          {(orders.length > 0 || allOrders.length > 0) && <motion.div initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
          ...
          {/* Orders Table */}
          {displayOrders.length > 0 && <motion.div initial={{
          opacity: 0,
          y: 20
        }} animate={{
          opacity: 1,
